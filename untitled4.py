...
def expert_system(age, bmi, glucose, insulin, genetic_factor):
    if age >= 40 and bmi >= 30 and glucose >= 140 and insulin >= 100 and genetic_factor == 1:
        return "Positive"
...
def main():
    ...
    genetic_factor = dataset[:, -2].reshape(-1, 1)
    ...
    test_data = np.array([48, 34.9, 125, 82]).reshape(1, -1)
    test_data = (test_data - X_mean) / X_std
    genetic_factor_test = np.array([1]).reshape(1, -1)
    ...
    expert_prediction = expert_system(age, bmi, glucose, insulin, genetic_factor_test)
